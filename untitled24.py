# -*- coding: utf-8 -*-
"""
Created on Sun Jun 26 14:17:30 2022
...
df = pd.read_csv('C:/Users/parkh/FJSP.csv',index_col=(0))
print(df)
class Ega_Scheduling():
    ...
    def crossover_operater(self, mom_cho, dad_cho):
        ...
        # 일점 교차를 사용함
        cut1=random.randint(0, self.params['RANGE']-4)
        cut2=cut1+3
    ...
    def replacement_operator(self, population, offsprings):
        ...
        for i in range(self.params["NUM_OFFSPRING"]):
            population.append([offsprings[i],
                               self.enumaration_TotalTardiness(df, offsprings[i])])
    ...
    def search(self):
        ...
        for i in range(self.params["POP_SIZE"]):
            ...
            for j in self.params["list_seq"]:
                chromosome.append(j)
            fitness=self.enumaration_TotalTardiness(df, chromosome)
...
for i in range(100):
    if __name__ == "__main__":
        ...
        total_tardiness += ga.search()
endtime = datetime.now()
elapsed_time = endtime-starttime
total_elapsed_time = 0
total_elapsed_time += elapsed_time.total_seconds()
print("100회 실행 평균 최적 tardiness: ", total_tardiness/100)
print("100회 평균 총 걸린시간: ", total_elapsed_time/100)